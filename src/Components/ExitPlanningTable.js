...
function ExitPlanningTable (props) {
  ...
  const onChange = (e, row) => {
    ...
    const newRows = rows.map(row => {
      if (row.id === id) {
        let sellAmount = row.sellAmount
        let remainingAmount = row.remainingAmount
        let sellingPoint = row.sellingPoint

        if (name === 'percentToSell') {
          let previousRemainingAmount = props.holdings

          if (row.id !== 0) {
            previousRemainingAmount = rows[id - 1].remainingAmount
          }

          sellAmount = (value * 0.01) * props.holdings
        ...
        const profit = sellAmount * sellingPoint
